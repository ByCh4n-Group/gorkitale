...
pub fn update(ctx: &mut Context, state: &mut GameState) -> tetra::Result {
    ...
    if state.player.pos.x > SCREEN_WIDTH as f32 || (state.player.pos.x < 0.0 && state.world.current_stage > 1) {
        if !state.fade_out {
            state.fade_out = true;
        }
    ...
    if state.fade_out {
        ...
        if state.fade_alpha >= 1.0 {
            ...
            if let Some(user) = &mut state.system.current_user {
                ...
                if let Some(idx) = state.system.users.iter().position(|u| u.username == user.username) {
    ...
    if state.world.current_stage == 3 {
        ...
        if state.player.pos.x >= door_rect.x && state.player.pos.x <= door_rect.x + door_rect.width &&
           state.player.pos.y >= door_rect.y && state.player.pos.y <= door_rect.y + door_rect.height {
             if input::is_key_pressed(ctx, Key::F) {
...
pub fn draw(ctx: &mut Context, state: &mut GameState) -> tetra::Result {
    ...
    if state.world.current_stage == 3 {
        ...
        if let Some(bg_texture) = &state.world.bg_texture {
            ...
            bg_texture.draw(ctx, DrawParams::new()
                .position(Vec2::new(0.0, 0.0))
                .scale(Vec2::new(scale_x, scale_y))
                .color(if state.world.current_stage == 1 { Color::WHITE } 
                       else if state.world.current_stage == 2 { Color::rgb(0.8, 0.8, 1.0) } // Blueish tint
    ...
    if state.world.current_stage == 2 {
        ...
        if distance < 100.0 && !state.world.gaster_talking {
            ...
            text.draw(ctx, DrawParams::new()
                .position(Vec2::new(state.world.gaster_pos.x - width / 2.0, state.world.gaster_pos.y - 80.0))
    ...
    if state.world.current_stage == 1 {
        ...
        if distance < 120.0 {
            let prompt = if state.world.music_playing { "Press F to Stop Music" } else { "Press F to Play Music" };
            let mut text = Text::new(prompt, state.font.clone());
            let width = text.get_bounds(ctx).map(|b| b.width).unwrap_or(100.0);
            
            text.draw(ctx, DrawParams::new()
                .position(Vec2::new(state.world.musicbox_pos.x - width / 2.0, state.world.musicbox_pos.y - 60.0))
    ...
    if state.world.music_playing {
        ...
        light_rect.draw(ctx, DrawParams::new().color(state.world.disco_color.with_alpha(0.2)));
    }
    ...
    if state.world.current_stage == 1 {
        ...
        if distance < 120.0 {
            ...
            text.draw(ctx, DrawParams::new()
                .position(Vec2::new(state.world.sans_pos.x - width / 2.0, state.world.sans_pos.y - 80.0))
                .color(Color::rgb(1.0, 1.0, 0.0))
            );
        }
    }
    ...
    if state.world.current_stage == 2 {
        if state.world.rarity_alive {
            ...
            if distance < 120.0 && state.player.pos.x < state.world.rarity_pos.x {
                ...
                text.draw(ctx, DrawParams::new()
                    .position(Vec2::new(state.world.rarity_pos.x - width / 2.0, state.world.rarity_pos.y - 80.0))
                    .color(Color::RED)
                );
            }
        } else if state.world.rarity_stabbed_timer > 0.0 {
            ...
            text.draw(ctx, DrawParams::new()
                .position(Vec2::new(state.world.rarity_pos.x - width / 2.0, state.world.rarity_pos.y))
                .color(Color::RED)
            );
        }
    }
